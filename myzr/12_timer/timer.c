...
static int led_init(void)
{
	...
	/* 初始化led所使用的IO */
	gpio_request(timerdev.led_gpio, "led");		/* 请求IO 	"led":lable,给gpio起的名*/
	ret = gpio_direction_output(timerdev.led_gpio, 1);//默认输出高电平
	if(ret < 0) {
...
static long timer_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case CLOSE_CMD:		/* 关闭定时器 */
			...
			break;
		case OPEN_CMD:		/* 打开定时器 */
			spin_lock_irqsave(&dev->lock, flags);//自旋锁
			timerperiod = dev->timeperiod;//定时周期作为临界资源
			spin_unlock_irqrestore(&dev->lock, flags);//解锁
			mod_timer(&dev->timer, jiffies + msecs_to_jiffies(timerperiod));//使用此函数打开定时器并设置周期为timerperiod也是默认周期
			break;
		case SETPERIOD_CMD: /* 设置定时器周期 */
			spin_lock_irqsave(&dev->lock, flags);
			dev->timeperiod = arg;//周期赋新值
			spin_unlock_irqrestore(&dev->lock, flags);
			mod_timer(&dev->timer, jiffies + msecs_to_jiffies(arg));//msecs_to_jiffies是毫秒级
			break;
		default:
			break;
	}
...
/* 定时器回调函数,当定时周期到了后此函数会被调用 ，在驱动入口函数中进行赋值给&dev->tiemr*/
void timer_function(unsigned long arg)
{
	struct timer_dev *dev = (struct timer_dev *)arg;//在驱动入口函数中将timerdev传递给参数，也即ioctl函数中对应的timerdev
	static int sta = 1;
	int timerperiod;
	unsigned long flags;
...
static int __init timer_init(void)
{
	...
	timerdev.timer.function = timer_function;//设置定时回调函数为timer_function
	timerdev.timer.data = (unsigned long)&timerdev;//data为要传递给function的参数即timer_function中的arg
	return 0;
}
